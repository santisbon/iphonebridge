...
- **Incoming messages** appear as desktop notifications that clear themselves after ten seconds. Dismissing one instead means "I read this", and marks the message read on the iPhone; letting it expire means nobody looked, so it stays unread. Reading it on the iPhone closes the popup here. Read state otherwise travels one way, desktop to phone — see the README's limitations. Set `IPHONEBRIDGE_NOTIFY_EXPIRE_MS=0` in `~/.config/iphonebridge/local.env` for popups that sit there until dealt with.
...
- **App notifications dismiss both ways.** Clearing one on the iPhone removes its card from the app's Notifications tab and closes its desktop popup; dismissing a card in the app (hover ✕ or right-click → Dismiss) sends the ANCS negative action, which clears it from the iPhone's notification center too. ANCS addresses notifications per BLE connection, so a card older than the current connection can only be removed locally — the app removes it either way and the daemon reports which happened.
- **Incoming calls** raise a notification with **Answer / Decline** buttons that act on the call directly.
- **Phone vitals on the Status tab.** The iPhone's battery percentage comes from its Bluetooth battery service (with the coarse HFP 0–5 indicator as a fallback, shown with a ~), cellular signal, carrier and registration from oFono's HFP modem, and the model from the Device Information service — the raw identifier (`iPhone18,1`) translated to its marketing name through a table generated from libirecovery's device list, falling back to the identifier for models newer than the table. Signal shows as four bars, matching the phone's own status bar: the HFP indicator allows six levels, but iOS sends the bar count it displays. A desktop notification fires once per dip below `IPHONEBRIDGE_LOW_BATTERY` (default 20%).
...
- **AVRCP** (Audio/Video Remote Control Profile) — control the iPhone's playback and see what's playing. AVRCP addresses whichever player the phone currently has in front, not a named app, so podcasts and audiobooks arrive through it exactly as music does. BlueZ implements the whole protocol and publishes the phone's player on D-Bus; the daemon only watches those objects and proxies commands, so there is no protocol code of ours involved. The player exists only while the classic audio link is up, which in practice means while the iPhone plays audio through this computer. Album art rides a small OBEX image service inside the same AVRCP record: BlueZ reports each track's image handle, and the daemon fetches the cover through obexd's `bip-avrcp` session into a small on-disk cache.
...
- **ANCS (Apple Notification Center Service)**: Apple's BLE service carrying every app's notifications. Granted per device by iOS — current iOS asks through an allow-notifications prompt on the phone — and needs a true BLE bond plus an LE connection; the picky one (see Troubleshooting).
