...
- **Phone vitals on the Status tab.** The iPhone's battery percentage comes from its Bluetooth battery service (with the coarse HFP 0–5 indicator as a fallback, shown with a ~), cellular signal, carrier and registration from oFono's HFP modem, and the model identifier from the Device Information service. Signal shows as bars out of five — that is the resolution the phone actually reports over HFP. A desktop notification fires once per dip below `IPHONEBRIDGE_LOW_BATTERY` (default 20%).
